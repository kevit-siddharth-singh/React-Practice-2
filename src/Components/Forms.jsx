...
function Forms() {
  ...
  const [selectedDropdownLocal, setSelectedDropdownLocal] = useState("");
  // form validation
  ...
  const validateLocalStates = () => {
    ...
    if (
      emailLocal.trim() === "" ||
      emailValidate(emailLocal.trim()) === false
    ) {
  ...
  }, [selectedDropdownLocal]);

  useEffect(() => {
    if (emailLocal.trim() !== "") {
      if (emailValidate(emailLocal.trim())) {
        setEmailError("");
      } else {
        setValidated(false);
      }
  ...
  useEffect(() => {
    ...
    // console.log(validated);
  }, [languageError, emailError, firstNameError, lastNameError]);
    ...
    <>
      <section className="bg-white dark:bg-gray-900">
        <div className="lg:grid lg:min-h-screen lg:grid-cols-12">
          ...
          <main className="flex items-center w-full justify-center px-2 max-sm:px-7 py-8 sm:px-12 lg:col-span-7 lg:px-16 lg:py-12 xl:col-span-6 ">
            <div className="w-full lg:max-w-3xl ">
              ...
              <form className="mt-8 grid grid-cols-6 gap-6">
                ...
                <div className="col-span-6  sm:col-span-3  flex  items-end">
                  <Dropdown
                    value={selectedDropdownLocal}
                    selected={setSelectedDropdownLocal}
                  />
                </div>
                <div className="col-span-6  sm:col-span-3  flex  items-end">
                  {/* Email  error */}
                  {(emailError !== null || emailError !== "") && (
                    <p className={`text-red-500 text-sm py-0 `}>{emailError}</p>
                  )}
                </div>
                <div className="col-span-6  sm:col-span-3  flex  items-end">
                  {/* Email  error */}
                  {(languageError !== null || languageError !== "") && (
                    <p className={`text-red-500 text-sm py-0 `}>
                      {languageError}
                    </p>
                  )}
                </div>
